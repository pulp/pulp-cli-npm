...
def remote(ctx: click.Context, pulp_ctx: PulpCLIContext, /, remote_type: str) -> None:
    if remote_type == "npm":
        ctx.obj = PulpNpmRemoteContext(pulp_ctx)
    else:
...
remote.add_command(create_command(decorators=common_remote_create_options + npm_remote_options))
remote.add_command(
    update_command(decorators=lookup_options + common_remote_update_options + npm_remote_options)
)
